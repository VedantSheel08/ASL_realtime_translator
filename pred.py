...
class Application:
    ...
    def __init__(self):
        ...
        self.model = load_model('D:\\ASL- GS\\Model\\OriginalModel.h5')
        ...
        self.root = tk.Tk()
        self.root.title("ASL Real-Time Translator")
        self.root.protocol('WM_DELETE_WINDOW', self.destructor)
        self.root.geometry("1300x700")
        self.root.configure(bg="#F1ED6D")

        self.panel = tk.Label(self.root)
        self.panel.place(x=500, y=0, width=480, height=640)
        self.panel.configure(bg="#F1ED6D")

        self.panel2 = tk.Label(self.root)  # initialize image panel
        self.panel2.place(x=50, y=80, width=400, height=400)

        self.T = tk.Label(self.root)
        self.T.place(x=60, y=5)
        self.T.config(text="ASL Real-Time Translator : Vedant Sheel", font=("Great Vibes", 30, "bold"))
        self.T.configure(bg="#F1ED6D")
        self.panel3 = tk.Label(self.root)  # Current Symbol
        self.panel3.place(x=280, y=585)

        self.T1 = tk.Label(self.root)
        self.T1.place(x=10, y=580)
        self.T1.config(text="Alphabet :", font=("Great Vibes", 30, "bold"))
        ...
        self.T3.config(text="Word Made:", fg="red",  font=("Great Vibes", 30, "bold"))

        self.T4 = tk.Label(self.root)
        self.T4.place(x=10, y=700)
        self.T4.config(text="Suggestions :", fg="black", font=("Great Vibes", 30, "bold"))
        ...
        self.speak.place(x=175, y=490)
        self.speak.config(text="Speak", font=("Great Vibes", 20), wraplength=100, command=self.speak_fun)

        self.clear = tk.Button(self.root)
        self.clear.place(x=50, y=490)
        self.clear.config(text="Clear", font=("Great Vibes", 20), wraplength=100, command=self.clear_fun)
    ...
    def video_loop(self):
        try:
            ...
            if hands:
                ...
                white = cv2.imread("D:\\ASL- GS\\white.jpg")
                ...
                if handz:
                    ...
                    for t in range(17, 20, 1):
                        cv2.line(white, (self.pts[t][0] + os, self.pts[t][1] + os1), (self.pts[t + 1][0] + os, self.pts[t + 1][1] + os1),
                                 (255, 255, 0), 3)
                    cv2.line(white, (self.pts[5][0] + os, self.pts[5][1] + os1), (self.pts[9][0] + os, self.pts[9][1] + os1), (255, 255, 0),
                             3)
                    cv2.line(white, (self.pts[9][0] + os, self.pts[9][1] + os1), (self.pts[13][0] + os, self.pts[13][1] + os1), (255, 255, 0),
                             3)
                    cv2.line(white, (self.pts[13][0] + os, self.pts[13][1] + os1), (self.pts[17][0] + os, self.pts[17][1] + os1),
                             (255, 255, 0), 3)
                    cv2.line(white, (self.pts[0][0] + os, self.pts[0][1] + os1), (self.pts[5][0] + os, self.pts[5][1] + os1), (255, 255, 0),
                             3)
                    cv2.line(white, (self.pts[0][0] + os, self.pts[0][1] + os1), (self.pts[17][0] + os, self.pts[17][1] + os1), (255, 255, 0),
                             3)

                    for i in range(21):
                        cv2.circle(white, (self.pts[i][0] + os, self.pts[i][1] + os1), 2, (0, 0, 0), 2)
                    ...
                    self.panel3.config(text=self.current_symbol, font=("Great Vibes", 30))
                    self.b1.config(text=self.word1, font=("Great Vibes", 20), wraplength=825, command=self.action1)
                    self.b2.config(text=self.word2, font=("Great Vibes", 20), wraplength=825,  command=self.action2)
                    self.b3.config(text=self.word3, font=("Great Vibes", 20), wraplength=825,  command=self.action3)
                    self.b4.config(text=self.word4, font=("Great Vibes", 20), wraplength=825,  command=self.action4)
            self.panel5.config(text=self.str, font=("Great Vibes", 30), wraplength=1025)
        except Exception:
...
#     model = load_model('D:\\ASL- GS\\Model\\OriginalModel.h5')
#     white = cv2.imread(test_image)
#     white = white.reshape(1, 400, 400, 3)
#     prob = np.array(model.predict(white)[0], dtype='float32')
#     print(np.argmax(prob))
# predict('D:\\ASL- GS\\AtoZ_3.1\\X\\10.jpg')
